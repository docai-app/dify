...
export const useChat = (
    ...
    const [lastResponseItem, setLastResponseItem] = useState<ChatItem>()
    const [unSpeakContent, setUnSpeakContent] = useState<any[]>([])
    const speakRef = useRef<any[]>([])

    // 使用正則表達式按標點符號和表情符號拆分緩衝區
    const regex = /([,.!?...，。！？...。🙂😃😂😍😊😭😢😡🤔👍👎✌️👌❤️💔])/;
    // const regex = /^[\u3000-\u303F\u2000-\u206F\!\@\#\$\%\^\&\*\(\)\-\=\+\[\]\{\}\\\|\:\;\'\\"\,\.\<\>\?\uFF01-\uFF0F]*$/
    ...
    const handleLasrItem = useCallback((item: ChatItem) => {
        setLastResponseItem(item)
    }, [])
    ...
    useEffect(() => {
        if (lastResponseItem) {
            // speakMessage(lastResponseItem?.content || '')
        }
    }, [lastResponseItem])
    ...
    const handleUpdateUnSpeakContent = useCallback((newList: any[]) => {
        setUnSpeakContent(newList)
        speakRef.current = newList
    }, [])

    const updateUnSpeakContent = useCallback((part: string) => {
        const newList = produce(
            speakRef.current,
            (draft) => {
                draft.push(part)
            })
        handleUpdateUnSpeakContent(newList)
    }, [handleUpdateUnSpeakContent])


    useEffect(() => {
        // if (unSpeakContent && unSpeakContent.length > 0)
        // console.log('unSpeakContent', unSpeakContent);
    }, [unSpeakContent])
        ...
        ssePost(
            ...
            },
            {
                ...
                onData: (message: string, isFirstMessage: boolean, { conversationId: newConversationId, messageId, taskId }: any) => {
                    ...
                    if (regex.test(message) && message.trim() != "") {
                        updateUnSpeakContent(buffer)
                        buffer = ''
                    }
